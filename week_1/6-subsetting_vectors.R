...
x[is.na(x)]
# Hint: Remember that is.na(x) tells us where the NAs are in a vector. So if we subset x based on that, 
...
# Output: Recall that `!` gives us the negation of a logical expression, so !is.na(x) 
# can be read as 'is not NA'. Therefore, if we want to create a vector called y that 
...
# FALSEs corresponding to values of y that are less than or equal to zero. What do you think y[y > 0] will give you?
# AnswerChoices: 
# A vector of all the positive elements of y
# A vector of all the negative elements of y
# A vector of all NAs
# A vector of length 0; A vector of TRUEs and FALSEs
# Hint: The logical expression y > 0 will give us TRUE for each element of y that
# is positive. Based on that, what do you think y[y > 0] will return?
...
# In this case, we're providing a vector of positive integers inside of the square brackets, which tells R to return only the elements of x numbered 1 through 10.
...
# Output: As you might expect, we get nothing useful. 
# Unfortunately, R doesn't prevent us from doing this. 
...
# gives us ONLY the 2nd and 10th elements of x, x[c(-2, -10)] gives us all elements of x
# EXCEPT for the 2nd and 10 elements.  
# Try x[c(-2, -10)] now to see this.
x[c(-2, -10)]
# Hint: Give x[c(-2, -10)] a try... I dare ya!

# Output: A shorthand way of specifying multiple negative numbers is to put the negative
# sign out in front of the vector of positive numbers. Type x[-c(2, 10)] to get the exact same result.
x[-c(2, 10)]
# Hint: Use x[-c(2, 10)] to simply things a bit. This could be a time saver if specifying many negative indexes.

# Output: So far, we've covered three types of index vectors -- logical, 
...
# Output: We can also get the names of vect by passing vect as an argument to the 
# names() function.
names(vect)
# Hint: Check out the results of names(vect).

# Output: Alternatively, we can create an unnamed vector vect2 with c(11, 2, NA).
vect2 <- c(11, 2, NA)
...
# Output: Now, let's check that vect and vect2 are the same by passing them as 
...
vect[bar]
vect["2"]
# Hint: If we want the element named "bar" (i.e. the second element of vect), 
# which command would get us that?
#   CorrectAnswer: vect["bar"]

# Output: Likewise, we can specify a vector of names with vect[c("foo", "bar")].
vect[c("foo", "bar")]
# Hint: Use vect[c("foo", "bar")] to get only the elements of vect named "foo" and "bar".

# Output: Now you know all four methods of subsetting data from vectors. 
